...
describe('sessionCacheBindingFor', () => {
  ...
  const storedFor = async (userId: string): Promise<string> => {
    const stored = await AsyncStorage.getItem(queryCacheKeyForUser(userId))

    return stored ?? ''
  }

  it('keys the session tree by the signed-in account, so one uid replacing another recreates it', () => {
    expect(sessionCacheBindingFor(USER_A).sessionKey).toBe(USER_A)
    expect(sessionCacheBindingFor(USER_A).sessionKey).not.toBe(sessionCacheBindingFor(USER_B).sessionKey)
  })
  ...
  it('keys a signed-out session with a value no Firebase uid can be', () => {
    expect(sessionCacheBindingFor(null).sessionKey).toBe(SIGNED_OUT_SESSION_KEY)
    expect(SIGNED_OUT_SESSION_KEY).not.toMatch(/^[A-Za-z0-9]+$/)
  })

  it('keys the same account the same way, so an unrelated re-render never remounts the tree', () => {
    expect(sessionCacheBindingFor(USER_A).sessionKey).toBe(sessionCacheBindingFor(USER_A).sessionKey)
  })

  it('busts a stored cache that was written for anybody else', () => {
    expect(sessionCacheBindingFor(USER_A).persistOptions.buster).toBe(USER_A)
    expect(sessionCacheBindingFor(USER_B).persistOptions.buster).toBe(USER_B)
    expect(sessionCacheBindingFor(null).persistOptions.buster).toBe('')
  })

  it("writes through the signed-in account's partition and nowhere else", async () => {
    activateQueryCachePartition(USER_A)

    const {persistOptions} = sessionCacheBindingFor(USER_A)

    await persistQueryClientSave({queryClient: seededClient(), ...persistOptions})

    await expect(storedFor(USER_A)).resolves.toContain('dailyMacros')
    await expect(AsyncStorage.getItem(LEGACY_QUERY_CACHE_KEY)).resolves.toBeNull()
    await expect(AsyncStorage.getItem(queryCacheKeyForUser(USER_B))).resolves.toBeNull()
  })
  ...
  it('stores only the whitelisted queries, whatever else the cache holds', async () => {
    ...
    const stored = await storedFor(USER_A)

    expect(PERSISTED_QUERY_KEYS).toContain('dailyMacros')
    expect(PERSISTED_QUERY_KEYS).not.toContain('mealPlanDay')
  ...
  it('hands an incoming account nothing the outgoing account stored', async () => {
    activateQueryCachePartition(USER_A)
    await persistQueryClientSave({queryClient: seededClient(), ...sessionCacheBindingFor(USER_A).persistOptions})

    activateQueryCachePartition(USER_B)

    const incomingClient = makeHydrationClient()

    await persistQueryClientRestore({queryClient: incomingClient, ...sessionCacheBindingFor(USER_B).persistOptions})
  ...
  it('stores and restores nothing at all while no identity is known', async () => {
    ...
    const {persistOptions} = sessionCacheBindingFor(null)
    ...
    await expect(AsyncStorage.getItem(LEGACY_QUERY_CACHE_KEY)).resolves.toBeNull()
    await expect(storedFor(USER_A)).resolves.toContain(USER_A)
  })
})
