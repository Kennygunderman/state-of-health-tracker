...
const applySetupStepSaveCacheEffects = (queryClient: QueryClient): void => {
...
export function buildSaveSetupStepMutationOptions(
  ...
  return {
    ...
    onSuccess: () => {
      applySetupStepSaveCacheEffects(queryClient)
    },
    // A rejected attempt owes the cache the same effects whenever the server may be holding the write.
    // `stale_revision` is proof it moved past the draft's `expectedRevision` — the repeated press landed the
