...
import {
    ...
    DINNER_MEAL_NAME,
    EMPTY_MEAL_SUBTITLE,
    EMPTY_MEAL_TITLE, LUNCH_MEAL_NAME,
    SERVINGS_TEXT,
...
const MealsScreen = ({ navigation }: any) => {
    ...
    const addFood = (mealName: string, mealId: string) => {
        navigation.push(Screens.ADD_FOOD, { mealName, mealId });
    };

    const onListItemPressed = (meal: Meal, foodItem: FoodItem) => {
        navigation.push(Screens.FOOD_DETAIL, { mealName: meal.name, mealId: meal.id, foodItem });
    };
    ...
    const renderMealEmptyState = () => (
        <SectionListFooter>
            ...
            <Text style={{ fontWeight: '200', marginTop: Spacing.XX_SMALL }}>{EMPTY_MEAL_SUBTITLE}</Text>
        </SectionListFooter>
    );
